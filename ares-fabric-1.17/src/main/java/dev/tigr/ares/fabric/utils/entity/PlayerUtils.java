...
public class PlayerUtils implements Wrapper {
    ...
    public static boolean isValidTarget(PlayerEntity player, double distance) {
        return !friendCheck(player)
                ...
                && player != MC.player
                && player != MC.world.getEntityById(Freecam.INSTANCE.clone != null ? Freecam.INSTANCE.clone : MC.player.getId());
    }
    ...
    public static boolean isInBurrow(PlayerEntity playerEntity) {
        BlockPos pos = new BlockPos(WorldUtils.getMiddlePosition(playerEntity.getX()), playerEntity.getY(), WorldUtils.getMiddlePosition(playerEntity.getZ()));
