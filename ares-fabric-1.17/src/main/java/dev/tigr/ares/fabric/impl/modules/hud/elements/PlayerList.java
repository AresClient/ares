...
public class PlayerList extends HudElement {
    ...
    public void draw() {
        List<PlayerEntity> players = MC.world.getPlayers().stream().filter(player -> !player.getName().equals(MC.player.getName())).collect(Collectors.toList());
        ...
        for(PlayerEntity player: players) {
            ...
            if(distance.getValue())
                drawString(String.valueOf((int) MC.player.distanceTo(player)), getX() + x, getY() + y, new Color(Integer.parseInt("808080", 16)));
