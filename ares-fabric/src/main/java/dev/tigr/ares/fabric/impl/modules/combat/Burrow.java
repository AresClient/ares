...
public class Burrow extends Module {
    ...
    private void switchToBlock() {
        ...
        if(newSelection != -1) {
            MC.player.getInventory().selectedSlot = newSelection;
    ...
    private void runSequence() {
        ...
        if(rubberband.getValue() == RubberbandMode.Packet || fakeJump.getValue()) {
            MC.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(initialPos.x, initialPos.y + fakeClipHeight.getValue(), initialPos.z, false));
        } else MC.player.jump();
