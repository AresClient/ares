    ...
    public void onRender3d() {
        if(result != null) {
            ...
            for(Vec3d point: result.getPoints()) {
                if(prevPoint != null) RENDERER.drawLine(point.x, point.y, point.z, prevPoint.x, prevPoint.y, prevPoint.z, 2, Color.WHITE);
                prevPoint = point;
            }

            if(box.getValue()) {
                GL11.glEnable(GL11.GL_BLEND);
                RenderGlobal.renderFilledBox(new AxisAlignedBB(new BlockPos(result.getHitVec())), 1, 0, 0, 0.6f);
            }
