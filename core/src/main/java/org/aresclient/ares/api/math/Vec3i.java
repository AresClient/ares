...
public interface Vec3i {
    static Vec3i create(int x, int y, int z) {
        return Ares.INSTANCE.creator.vec3i(x, y, z);
    }

    static Vec3i create(double x, double y, double z) {
        return Ares.INSTANCE.creator.vec3i((int) Math.floor(x), (int) Math.floor(y), (int) Math.floor(z));
    }

    static Vec3i create(Vec3d vec3d) {
        return Ares.INSTANCE.creator.vec3i((int) vec3d.getX(), (int) vec3d.getY(), (int) vec3d.getZ());
    }
    ...
    default float squareDistanceTo(int x, int y, int z) {
        int d0 = x - getX();
        int d1 = y - getY();
        int d2 = z - getZ();
        return d0 * d0 + d1 * d1 + d2 * d2;
