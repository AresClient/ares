...
//    private val LEGACY = org.aresclient.ares.Ares.MESH.loaderVersion.startsWith("1.12")
    ...
    inline fun render3d(callback: (Buffers, MatrixStack) -> Unit) {
        ...
        Ares.INSTANCE.minecraft.getRenderer().getCamera().also {
            matrixStack.projection()
                .set(Ares.INSTANCE.minecraft.getRenderer().getRenderStack().getProjectionMatrix())
    ...
    inline fun scissor(x: Float, y: Float, width: Float, height: Float, callback: () -> Unit) {
        val framebuffer = Ares.INSTANCE.minecraft.getFrameBuffer()
        val resolution = Ares.INSTANCE.minecraft.getResolution()
